...
public class AdaptiveTrackSelection extends BaseTrackSelection {
  ...
  public void updateSelectedTrack(
    ...
    temp = temp+ Long.toString(bufferedDurationUs)+", ";
    ...
    try{
      File f=new File(c.getExternalFilesDir(null).getPath()+"hhh.txt");
      if (!f.exists()){
        f.createNewFile();
      }
    ...
    for (int i = 0; i < length; i++) {
      if (nowMs == Long.MIN_VALUE || !isBlacklisted(i, nowMs)) {
        ...
        if (canSelectFormat(format, format.bitrate, effectiveBitrate)) {
          temp = temp+ Long.toString(format.bitrate);
          return i;
        } else {
  ...
  private long getTotalAllocatableBandwidth(long chunkDurationUs) {
   ...
   temp = temp+ Long.toString(cautiousBandwidthEstimate)+", ";
